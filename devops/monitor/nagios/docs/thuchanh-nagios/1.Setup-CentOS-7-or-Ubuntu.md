...
- [Hướng dẫn cài đặt Nagios Core 4.3.1 trên CentOS 7](#hướng-dẫn-cài-đặt-nagios-core-431-trên-centos-7)
    - [Table of contents](#table-of-contents)
    - [1. Chuẩn bị](#1-chuẩn-bị)
        - [1.1 Chuẩn bị môi trường cài đặt](#11-chuẩn-bị-môi-trường-cài-đặt)
        - [Mô hình cài đặt giám sát các host như sau:](#mô-hình-cài-đặt-giám-sát-các-host-như-sau)
        - [IP Planning](#ip-planning)
    - [2. Cài đặt Nagios trên Nagios Server](#2-cài-đặt-nagios-trên-nagios-server)
        - [2.1 Cài đặt các gói chuẩn bị cần thiết](#21-cài-đặt-các-gói-chuẩn-bị-cần-thiết)
        - [2.2 Tạo user cho Nagios](#22-tạo-user-cho-nagios)
        - [2.3 Cài đặt Nagios Core và Plugin](#23-cài-đặt-nagios-core-và-plugin)
        - [2.4 Khởi động Nagios Server](#24-khởi-động-nagios-server)
    - [3. Giám sát host/dịch vụ trên Linux](#3-giám-sát-hostdịch-vụ-trên-linux)
        - [3.1 Giám sát thông qua NRPE](#31-giám-sát-thông-qua-nrpe)
            - [3.1.1 Cài đặt NRPE trên Nagios Server](#311-cài-đặt-nrpe-trên-nagios-server)
            - [3.1.2 Cài đặt NRPE trên host cần giám sát](#312-cài-đặt-nrpe-trên-host-cần-giám-sát)
            - [3.1.3 Thêm thông tin host trên Nagios Server](#313-thêm-thông-tin-host-trên-nagios-server)
            - [3.1.4 Kiểm tra trên Web UI của Nagios Server](#314-kiểm-tra-trên-web-ui-của-nagios-server)
            - [3.1.5 Fix lỗi](#315-fix-lỗi)
                - [3.1.5.1 NRPE: Unable to read output](#3151-nrpe-unable-to-read-output)
        - [3.2 Giám sát dịch vụ MySQL](#32-giám-sát-dịch-vụ-mysql)
        - [3.3 Giám sát dịch vụ RabbitMQ](#33-giám-sát-dịch-vụ-rabbitmq)
...
###### 3.1.5.1 NRPE: Unable to read output

add to visudo

```conf
nagios ALL=(xuananh) NOPASSWD: /deploy/plusfun/monitor/nrpe/check-script/check-plusfun-api.py
nagios ALL=(xuananh) NOPASSWD: /deploy/plusfun/monitor/nrpe/check-script/check_mysql_replication.sh
```
